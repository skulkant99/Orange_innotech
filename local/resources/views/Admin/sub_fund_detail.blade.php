...
<div class="modal" id="ModalAdd"  role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document"  style="max-width:70%;max-height:70%;">
        <div class="modal-content">
            <form id="FormAdd">
                <input type="hidden" name="sub_fund_title_id" value="{{$id}}" >
                ...
                <div class="form-group">
                    <label for="add_detail_en">รายละเอียด(TH)</label>
...
<script>
     ...
     var TableList = $('#TableList').dataTable({
        "ajax": {
            "url": url_gb+"/admin/SubFundDetail/Lists",
            "data": function ( d ) {
                d.sub_fund_title_id = {{$id}};
    ...
    $('body').on('click','.btn-edit',function(data){
        ...
        $.ajax({
            method : "GET",
            url : url_gb+"/admin/SubFundDetail/"+id,
            dataType : 'json'
        }).done(function(rec){
    ...
    $('#FormAdd').validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                method : "POST",
                url : url_gb+"/admin/SubFundDetail",
                dataType : 'json',
                data : $(form).serialize()
            }).done(function(rec){
                btn.button("reset");
    ...
    $('body').on('click','.btn-delete',function(e){
        ...
        swal({
            ...
            $.ajax({
                method : "POST",
                url : url_gb+"/admin/SubFundDetail/Delete/"+id,
                data : {ID : id}
            }).done(function(rec){
                if(rec.status==1){
                    swal(rec.title,rec.content,"success");
